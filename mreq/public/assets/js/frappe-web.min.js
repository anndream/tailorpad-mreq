...
console.error?console.error(err):console.log(err);}catch(e){console.log(data.exc);}
if(data._server_messages){var server_messages=(JSON.parse(data._server_messages||'[]')).join("<br>");if(opts.error_msg){$(opts.error_msg).html(server_messages).toggle(true);}else{frappe.msgprint(server_messages);}}}else{if(opts.btn){$(opts.btn).addClass("btn-success");setTimeout(function(){$(opts.btn).removeClass("btn-success");},1000);}}
