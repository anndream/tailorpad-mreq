...
frappe.SalesForm = Class.extend({
	init: function(wrapper, form_type, search_key, si_no){
		...
		this.field_list = {
			...
			'Tailoring Item Details':[
				...
				['Service', 'Link', 'Price List','tailoring_price_list',1], 
	...
	render_child: function(fields, form_type, key){
		...
		$('[data-fieldname="fabric_code"]').change(function(){
			frappe.call({
				...
				callback: function(r){
					console.log(r.message)
					$('[data-fieldname="fabric_rate"]').attr('value', r.message.fc)
					$('[data-fieldname="width"]').attr('value', r.message.fw)
					me.set_tot()
				}
			})
		})
	...
	set_tot:function(){				
		...
		$('[data-fieldname="tot_amt"]').val(flt($('[data-fieldname="tailoring_qty"]').val()) * flt($('[data-fieldname="tailoring_rate"]').val()) + flt($('[data-fieldname="fabric_qty"]').val()) * flt($('[data-fieldname="fabric_rate"]').val()))
	},
	type_checker:function(form_type, key){
		if(form_type == 'new')
			return true;
